...
After a potential detection of a stochastic GW background by PTA observatories EPTA, InPTA, NANOGrav, PPTA, and CPTA in 2023, constrains in the merger rates of MBHBs have been possible. We can use this new information to update the expressions from W22 and K16, as they are now out of date. We do this by normalising them to the results for the merger rates from Barausse \textit{et al}.\ (2023, B23)~\cite{barausse2023implications}, which takes these constraints into account, Fig.~\ref{fig:mergerrates_wang_barausse}. Within their semi-analytical model, B23 compute the merger rate per unit redshift with the expression,
\begin{equation}
\frac{d^{2}N}{dz\,dt}
=
\frac{4\pi\,c}{\Delta z}
...
\FloatBarrier
\subsubsection{Source-frame Total Mass}
\label{subsubsec:paramcomponentmasses}
...
&= A~M^{\alpha}, & M_{\min} \le M \le M_{\mathrm{max}},
...
where $\alpha$, $\alpha_1$, and $\alpha_2$ are the hyper-parameters that define the slopes of the mass distributions for $M$, and $A$, $A_1$, and $A_2$ are the normalisation factors. 

The continuity condition of Eq.~\eqref{eq:mass_distr_Q3} is
...
The population distributions for the popIII, Q3d, and Q3nod population models are shown in Fig.~\ref{fig:pM_popmodels}.
...
$M_{\mathrm{break}}$ & Powerlaw break source-frame total mass ($\mathrm{M}_{\odot}$). & -- & $1\times10^{6}$ & $3\times10^{5}$ \\
$\alpha$ & Powerlaw index for source-frame total mass of the popIII model. & $-1.30$ & -- & -- \\
$\alpha_{1}$, $\alpha_{2}$ & Powerlaw indexes for source-frame total mass of the Q3d and Q3nod models. & -- & $-0.20$,$-1.75$ & $-0.20$,$-1.90$ \\
...
$q_{\mathrm{break}}$ & Powerlaw break for the mass ratio for the popIII model. & 0.88 & -- & -- \\
$\alpha_{q}$ & Powerlaw index for the mass ratio of the Q3nod model. & -- & -- & $-0.60$ \\
$\alpha_{q,1}$, $\alpha_{q,2}$ & Powerlaw indexes for the mass ratio of the popIII model. & $0.80,-0.14$ & -- & -- \\
\bottomrule
\end{tabular}
...
where $\mathcal{N}=\int_{z_{min}}^{z_{max}}\frac{dN}{dtdz}dz$ is the normalisation factor. The redshift distribution for the popIII, Q3d, and Q3nod population models are shown in Fig.~\ref{fig:pz_popmodels}.
...
\begin{figure}[h]
    ...
    \end{subfigure}    
    \caption{$p(z)$ computed from the merger rates from Wang \textit{et al}.\ (2021) normalised to the intrinsic merger rates from Barausse \textit{et al}.\ (2023)}
    \label{fig:pz_popmodels}
\end{figure}

\FloatBarrier
\subsubsection{Spins}
\label{subsubsec:paramspins}
...
We chose the beta distribution because spins' span is $(0,1)$, and follows closely the distribution obtained in B23. The spin distributions are shown in Fig.~\ref{fig:pa_popmodels}.
...
\label{subsubsec:parammassratio}
...
&= A~q^{\alpha_{q}}, & q_{\min} \le q \le q_{\mathrm{max}},
...
&=
\begin{cases}
 A_{1}~q^{\alpha_{q,1}}, & q_{\min} \le q \le q_{\mathrm{break}},\\[6pt]
 A_{2}~q^{\alpha_{q,2}}, & q_{\mathrm{break}} < q \le q_{\max}.
\end{cases}
\label{eq:mass_ratio_popIII}
\end{align}
The hyper-parameter values are shown in Table~\ref{tab:hyper-parameters_values}, and the distributions in Fig.~\ref{fig:pq_popmodels}.
