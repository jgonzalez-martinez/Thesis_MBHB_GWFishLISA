...
As we have stated previously, there is strong evidence that MBHs and galaxies co-evolve. Approaches to follow this co-evolution include cosmological simulations, theoretical models, and phenomenological models. Cosmological simulations can be carried out in two fashions: Grid-based Eulerian simulations, and particle-based hydrodynamical simulations (Smoothed Particle Hydrodynamics, SPH). While cosmological simulations give a robust track of the evolution of MBHs and their host galaxies, they are computationally expensive. On the theoretical side, there exist analytical and semi-analytical models. Theoretical models are limited in their reach as far as the physics that is included in the modelling. Semi-analytical models provide simple enough prescriptions, tunable up to a few parameters, while being computationally affordable. Phenomenological, or parametric, models consist of models that fit observations or the predictions on numerical simulations. In this work, we use parametric models because they are flexible and allow us to fit the results of semi-analytical modes without requiring us to model explicitly the physical mechanisms that generate the corresponding distributions, and because they can be sampled quickly and at low computational cost.

We use parametric forms for the merger rate of MBHBs, $R(z)$, and for a set of binary parameters: source-frame total mass $M = m_{1} + m_{2}$, redshift $z$, spin magnitudes $a_1$ and $a_2$, and mass ratio $q~(=m_2/m_1)$. We are using the results from the semi-analytical simulations of Barausse \textit{et al}.\ (2023, B23)~\cite{barausse2023implications}, and fit our parametric models to the results of B23.
...
After a potential detection of a stochastic GW background by PTA observatories EPTA, InPTA, NANOGrav, PPTA, and CPTA in 2023, constrains in the merger rates of MBHBs have been possible. We normalised the expressions from Wang \textit{et al}.\ to the results for the merger rates from Barausse \textit{et al}.\ \cite{barausse2023implications}, which takes into account these constraints, Fig.~\ref{fig:mergerrates_wang_barausse}. To normalise our merger rates, eqs.~\eqref{eq:mergerrates_wang}, we used the expression in Barausse \textit{et al}.\ for the merger rate as a function of redshift, which is
\begin{equation}
\frac{d^{2}N}{dz\,dt}
=
\frac{4\pi\,c}{d z}
...
It has been shown that cosmological simulations lack resolution in high-mass dark matter halos at high redshifts~\cite{klein2016science}. This underestimates the number of MBHB mergers in the Universe. In their study, Klein \textit{et al}.\ linearly extrapolated their results at low redshifts, to higher redshifts to compensate for the lack of resolved high-mass halos at high redshifts. The number of mergers they obtained are shown in Table~\ref{table:intrinsic_merger_rate_klein}.

\FloatBarrier
\begin{table}[t]
\centering
\caption{Intrinsic MBHB merger rates (events yr$^{-1}$ over cosmic history) for the Klein \textit{et al}.~\cite{klein2016science}\ population models. Extrapolated rates correct for under-resolved high-mass halos.}
\begin{tabular}{lrrl}
\hline
...
LS & popIII        & $352.58$ & Barausse \textit{et al}. (2023). Used to normalise Eqs.~\ref{eq:mergerrates_wang}.\\
...
HS & Q3d        & $18.38$ & Barausse \textit{et al}.\ (2023). Used to normalise Eqs.~\ref{eq:mergerrates_wang}.\\
