...
LISA is composed by three spacecrafts (s/c) each of them housing two test masses. s/c and test masses are set to free-fall orbiting the Sun. In order for them to experience a free-fall state, s/c and test masses need to be exempt from external non-gravitational forces. Space is a suitable environment to set s/c and test masses in this setting since it is quiet enough from non-gravitational forces such as solar radiation pressure or cosmic particles. The importance of (???) s/c and test masses in this environment is evident since spurious non-gravitational forces can be mistaken as real strains from passing GWs. Broadly speaking, laser interferometry relies on two key aspects \cite{prince2011lisa}: 1) Measurement of length changes between two test masses, and 2) A measurement system that processes these measurements. Non-gravitational strains need to reduced as much as possible and to be smaller than the GW strain so that they do not have significant influence on the length change measurement from GWs. 

The LISA detector comprises three s/c trailing the Earth orbiting the sun Fig. ~\ref{fig:LISAobs}. There are three key aspects behind the LISA design concept. 1) A set of three s/c with unique orbits. LISA is comprised by three s/c arranged in a triangular shape, or constellation. Each s/c orbit the Sun in a unique manner, so that the whole constellation orbits the sun resembling a cart-wheel motion. The constellation orbits the Sun trailing the Earth at an angle of $20^{\circ}$. The plane of the constellation is tilted 60º relative to the plane of the ecliptic. At each vertex of the constellation lies a s/c. The spacecrafts house the test masses, as well as measurement equipment, Fig~\ref{fig:LISAdetail}, which are in free-fall state. Measurements are made monitoring the length change between test masses.
The length change due to passing GWs between two test masses in one arm are partitioned in three measurements: The measurement between s/c and the measurement between test mass and the s/c for each end-point of the arm, Fig.~\ref{fig:LISAarm}. By combining these three measurements, the length change between test masses at each end-point of each arm can be done. Additionally, the noise induced by each partition is considered negligible for LISA \cite{prince2011lisa} so it does not affect the test mass length change significantly.
\begin{figure}[htbp]
    \centering
...
2) The implementation of "drag-free" operation on the s/c and test masses. Each s/c electrostatically houses a test mass for each arm and play the role of shielding the test mass against non-gravitational effects. As the test mass is kept in free-fall, the s/c needs to maintain its position relative to the test mass. This is done by sensors that measure the position of the s/c relative to the test mass and thrusters to correct its position if needed. The test masses posses sensors that monitor any disturbances in their free-fall motion as well, and corrections are made by means of small electrostatic forces imparted by the electrostatic housing if needed. 3) Distance measurement system. Classic laser interferometry relies on mirrors to send a laser beam forward and back to measure length strains. This approach is not feasible for LISA due to the much larger arms. Instead, the lasers in LISA work in a "transponder" fashion \cite{prince2011lisa}: For each arm, one s/c sends a laser beam to the other end's s/c. When the laser reaches the opposite end, the laser is "locked-down", i.e. its phase is measured and stored at the arriving point to be replicated. At this end, a laser with the replicated phase is send back to the original end. When the laser is received by the original end, its phase is compared with the phase of the original end. Differences in the phase of the forward and back lasers are compared determining whether a length change has occurred.
...
In general, sensitivity curves of GW detectors show the strain (or amplitude) sensitivity of the detector, in units of $1/\sqrt{\mathrm{Hz}}$ in the frequency domain, to detect spacetime fluctuations, due to noise and passing GWs. This is the curve typically shown when compared with GWs signal strains in a strain vs frequency space. Other names for the sensitivity curve are amplitude spectral density (ASD), spectral strain sensitivity and spectral amplitude. The sensitivity curve is obtained from the noise power spectral density (PSD) which is a measure of the noise strength as a function of frequency. The noise PSD can be expressed by \cite{robson2019construction},
\begin{equation}
...
\label{eq:psdLISA}
\end{equation}
where $L = 2.5~\mathrm{Gm}$, and $f_{\ast} = c/(2 \pi L) = 19.09~\mathrm{mHz}$ and is called the transfer frequency. The noise sources that affect LISA are included in the terms $P_{\mathrm{OMS}}$, $P_{\mathrm{acc}}$ and $S_{c}$, which are the noise associated with the optical metrology system, the noise due to test masses acceleration fluctuations and the confusion noise that comes from unresolved sources in the galaxy, respectively. In the next lines we will show how to compute the noise PSD in eq. \eqref{eq:psdLISA}. In treatment presented here, the noise PSD is expressed by both $S_n$ and $P_n$. 
However, $P_n$ does not include the detector response function which relates the PSD of the signal to the PSD of the signal as recorded by the detector. We combine the detector response function and $P_n$ to obtain $S_n$ with the expression,
\begin{equation}
...
\label{eq:noiseLISA}
\end{equation}
and $\mathcal{R}(f)$ is the detector response function. The gravitational wave amplitude recorded by the detector, $\Tilde{h}(f)$ and the polarisations of the signals are related through \cite{robson2019construction},
\begin{equation}
...
There exist many software programs or \textit{pipelines} in GW astronomy that calculate the posterior PDF of the GW parameters. One popular pipeline is \textit{LALinference}, which was used to detect GW150914 \cite{abbott2016observing}, the first detection of GWs. 
\textit{LALinference} is based on parallel tempering Markov Chain Monte Carlo \cite{rover2007coherent}, which generates posterior samples in a multi-dimensional parameter space, and nested sampling  which calculates the evidence, and the posterior as a by-product \cite{skilling2006nested}. \textit{LALinference} output's are posterior samples for all parameters that characterise the GW signal. Other \textit{pipeplines} used in GW astronomy are rapidPE \cite{lange2018rapid} and BILBY \cite{ashton2019bilby} that incorporate Bayesian methods as well.

\subsubsection{Posterior Distributions}

GW parameter estimation pipelines' output are posteriors in a multidimensional parameter space for all parameters. This means that these posteriors include the correlations between parameters as well as the posteriors for each parameter. An example of this is shown in fig.  where the $2-\mathrm{dimensional}$ \textit{joint} posterior for the component masses of a BHB has been computed, showing the correlations between the two parameters. The other two panels show $1-\mathrm{dimensional}$ \textit{marginalised} posteriors for each of the parameters.
