...
Massive black hole binaries (MBHBs) provide a direct window into the coevolution of galaxies and their central black holes, and into the assembly of massive black holes (MBHs) through accretion and mergers \cite{kormendy2013coevolution,volonteri2010formation,begelman1980massive,klein2016science}. Despite this importance, key aspects of their astrophysical origin remain uncertain. On large scales, we still lack a complete understanding of how two MBHs formed in a galaxy merger are brought together from kiloparsec separations to parsec scales where a bound binary forms, and subsequently into the GW-driven regime leading to coalescence. At earlier times, uncertainties in MBH seeding, whether seeds originate from Population III (Pop III) remnants, runaway stellar collisions in dense clusters, direct collapse of supermassive stars (SMS), or primordial black holes, propagate into predictions for MBH demographics and MBHB merger rates.
...
Understanding how MBHs form and grow is central to understanding galaxy evolution, and it sets the initial conditions for MBHB formation and GW emission. In this section, we review the main proposed seeding mechanisms: PopIII remnants, dense star-cluster dynamics, direct collapse (SMS), and primordial black holes, followed by the dominant growth mechanisms, namely gas accretion and mergers. We then discuss the empirical and theoretical links between MBHs and their host galaxies, including scaling relations and their possible evolution with redshift.
...
There are two hurdles for Pop III MBH light seeds for growing up as massive as the quasars we observe at $z\sim6$--$7$ \cite{amaro2023astrophysics}: 1) If the seeds do not form at the center of the galaxies, they need to reach it efficiently, and 2) The process of accretion of gas needs to be efficient, with accretion rates reaching Eddington rates, or even super-Eddington rates. These accretion rates would require more complex mechanisms of accretion and energy transport, with the caveat that they would imply high radiative feedback from the accretion flow, decreasing the efficiency of accretion.

Recent JWST spectroscopy is now beginning to directly probe these early growth phases, revealing accreting BH candidates at redshifts $z>4$ and providing initial constraints on BH masses and Eddington ratios \cite{larson2023ceers,maiolino2024small,harikane2023jwst}.
Reaching such high accretion rates likely requires non-standard inflow and energy-transport mechanisms, but the associated radiative feedback can in turn suppress inflow and reduce the net growth efficiency.
...
Observational evidence supports the idea of MBHs being in the centre of most galaxies \cite{kormendy2013coevolution}. There is also evidence that a growth channel of galaxies is through episodes of mergers \cite{fakhouri2010merger,o2021emerge}. This points to the conclusion that there should exist populations of MBH binaries (MBHBs) in the Universe and that their mergers could be detected through their emission of GWs by LISA \cite{klein2016science,dayal2019hierarchical,chen2020dynamical}.

So far, our knowledge of MBHBs has largely come from EM observations, cosmological simulations, and theoretical tools, but there are still some missing pieces of the puzzle of their formation and evolution that remain uncertain. Low-frequency GW observations will add a new, complementary probe: PTAs are already constraining the nanohertz stochastic background from the most massive binaries, while LISA will detect individual MBHB coalescences in the millihertz band and measure their parameters precisely. Together, PTAs and LISA will help address these gaps by constraining MBH seeding and growth across cosmic time, and by testing how efficiently binaries are driven from galaxy scales into the GW-driven regime. In this context, two key questions arise \cite{amaro2023astrophysics}: 1) What are the mechanisms that bring together two MBHs at kiloparsec scales down to the separation where GW emission drives the evolution at $\sim10^{-5} \mathrm{pc}?$ and 2) Are these mechanisms always efficient enough so that a galaxy merger ends up with a MBHB merger? 
...
\label{eq:dedt}
\end{equation}
When considering PN coefficients up to order 3.5, or higher, the Keplerian variables of the orbit are not constant anymore, but rather oscillate \cite{will2014confrontation,mannerkoski2019gravitational,memmesheimer2004third}. When including spins, both magnitude and direction shape the dynamics of the binaries as well as the GWs emitted.
The time a binary takes to merge was derived by Peters \cite{peters1964gravitational}, and its functional form is \cite{amaro2023astrophysics}, i.e.,
\begin{equation}
t_{\mathrm{Peters}} \propto\frac{(1+q)^{2}}{q}\frac{a^{4}_{0}}{M^{3}}\frac{1}{f(e_{0})},
\label{eq:peterstimescale}
\end{equation}

Peters formula, Eq.~\eqref{eq:peterstimescale}, assumes orbit decay, Keplerian orbits, and orbital decay only through GW emission, i.e.\ where $a$ and $e$ vary slowly. Zwick \textit{et al}.\ \cite{zwick2020improved} proposed a corrected formula in which these assumptions are relaxed while adding a new spin-dependent correction and has the functional form \cite{amaro2023astrophysics},
\begin{equation}
...
M/c^{2}$, $R(e_{0})=8^{(1-\sqrt{1-e_{0}})}$, and $s_{1}\equiv S_{1}\cos\theta$, with $S_{1}$ being the spin magnitude of the more massive MBH and $\theta$ the angle between the MBH spin vector and the orbital total angular momentum vector. This formula refines the inspiral time-scale which have impact on merger-rate estimates.
...
For MBHBs with smaller orbital separations compared to the ones involved in optical spectroscopic surveys, Reynolds~\cite{reynolds2015measuring,reynolds2021observational} used the broad iron fluorescence emission lines observed at $\sim6.4~\text{keV}$ in X-rays in the accretion flow of many AGNs with masses of $~10^{6}~\mathrm{M}_{\odot}$. However, this observation is limited to AGNs at lower redshifts ($z<0.1$) compared to LISA's targets, so high sensitivity, high redshift X-ray facilities,  like ESA's NewAthena~\cite{cruise2025newathena}, will be suitable to take full advantage of this approach. 
...
\paragraph{}
Although multimessenger astronomy with GWs will revolutionise our current MBH paradigm, substantial uncertainties remain in the predicted EM counterparts of MBHBs, such as their light curves and spectra, and in the structure and properties of the environment surrounding MBHBs.
...
\paragraph{}
To understand the populations of MBHs in the Universe, we need to survey MBHs across cosmic time, from early to late Universe, over a wide range of masses, and in both isolated and binary systems. Indeed, GW observations by themselves, particularly with LISA, will provide direct and precise constraints on MBHBs in the range of $10^{4}~\mathrm{M}_{\odot}$ and $10^{9}~\mathrm{M}_{\odot}$, including their masses, redshift, and spins~\cite{amaro2017laser}. Where EM information is available, it can complement and refine these GW constraints by probing the host galaxy and the surrounding environment, thereby enabling a more complete physical interpretation and strengthening population studies.
...
\paragraph{Current knowledge about MBHs with current and future missions.}

While LISA will probe MBHBs in the millihertz band, PTAs target GWs at much lower frequencies, in the nanohertz regime. PTAs search for GWs by monitoring millisecond pulsars over many years and measuring deviations ("timing residuals") from the expected pulse arrival times. A detection is made when a common signal is present across many pulsars, and the cross-correlations between their timing residuals follow the characteristic quadrupolar correlation signature~\cite{hellings1983upper} expected for a GW background. The main targets of PTAs are a stochastic GW background, generated by the superposition of many unresolved MBHB signals, and potentially, individually resolvable MBHBs with masses of $10^8$--$10^{10}~\mathrm{M}_{\odot}$ at low redshift ($z \lesssim 1.5$)~\cite{sesana2010measuring}. 

Independent PTA collaborations (NANOGrav\cite{arzoumanian2020nanograv}, EPTA/InPTA\cite{antoniadis2023second}, PPTA\cite{reardon2023search}, and CPTA\cite{xu2023searching}) reported potential evidence for a stochastic background consistent with expectations from a population of MBHBs. However, other sources, such as cosmic strings~\cite{ellis2021cosmic}, could also produce a similar spectrum. In this case, longer baselines and larger PTAs will be needed to distinguish between scenarios and strengthen interpretation.
...
Each LISA spacecraft contains two test masses (one associated with each interferometric arm) enclosed within an electrostatic housing. The test masses act as the nearly free-falling reference bodies, while the spacecraft provides shielding from non-gravitational disturbances such as solar radiation pressure. To keep the spacecraft centered on the test masses, LISA measures the relative spacecraft-test-mass displacement with sensors and uses micro-thrusters to apply forces to the spacecraft. 
...
We use parametric forms for the merger rate of MBHBs, $R(z)$, and for a set of binary parameters: source-frame total mass $M = m_{1} + m_{2}$, redshift $z$, spin magnitudes $a_1$ and $a_2$, and mass ratio $q = m_2/m_1$. 
\\
%\FloatBarrier
%\subsubsection{Intrinsic merger rates}
%\label{subsubsec:mergerrates}
...
\begin{equation}
  ...
  \label{eq:mergerrate}
\end{equation}
where $dV_c/dz$ is the comoving volume element, $\mathcal{R}(z)$ is the merger rate density, and $1/(1 + z)$ converts to observer frame. The merger rate density $\mathcal{R}(z)$ is an intrinsic, local quantity that reflects the number of mergers per comoving volume, per comoving time, and depends on the astrophysical model. On the other hand, the merger rate $R(z)$ is the global number of mergers that happen in the Universe per unit detector-frame time.

In this work, we use fitted expressions for the merger rates from Wang \textit{et al}. (2022; W22)~\cite{wang2022forecast}, which are piecewise approximations for the merger rates from Klein \textit{et al}.\ (2016; K16)~\cite{klein2016science} in their Fig. 3. These expressions are
...
Within their semi-analytical model, B23 compute the merger rate per unit redshift with the expression,
\begin{equation}
\frac{d^{2}N}{dz\,dt}
=
\frac{4\pi\,c}{\Delta z}
...
$\mathrm{d}\dot N/\mathrm{d}z$ (per unit redshift and per unit observer time) to define the expected
number of events over a mission duration $T$. The expected number of intrinsic mergers is
\begin{equation}
\mu(T)=T\int_{0}^{z_{\max}} \frac{\mathrm{d}\dot N}{\mathrm{d}z}\,\mathrm{d}z,
...
The merger rate evolution with redshift, and the results of the merger rates for our three population models are shown in Fig.~\ref{fig:mergerrates_wang_barausse}, and in Table~\ref{table:intrinsic_merger_rate}, respectively.
...
\centering
\caption{Mergers during a \textbf{4-year} mission. The rate $R(z)$ includes the Poisson counting uncertainty, $\sigma_{R(z)}=\sqrt{N}/T$.}
\footnotesize
...
\toprule
\textbf{Model} & {Total mergers $N_{\rm intr}$ (4 yr)} & {Rate $R(z)$ (yr$^{-1}$)} \\
\midrule
...
\begin{align}
p(z) =
\frac{1}{\mathcal{N}} \frac{d N}{dtdz},
\label{eq:redshift_distr}
\end{align}

where $\mathcal{N}=\int_{z_{min}}^{z_{max}}\frac{dN}{dtdz}dz$ is the normalisation factor. The redshift distributions for the popIII, Q3d, and Q3nod population models are shown in Fig.~\ref{fig:row_pz}.
\\
...
\begin{figure}[t]
    ...
    \end{subfigure}
    % Global caption 
    \caption{Distributions of mergers from the synthetic detected catalogues generated by adding Gaussian noise to the true parameter values in order to simulate measurement uncertainties. Columns correspond to the three astrophysical models, PopIII, Q3d, and Q3nod. Rows show, from top to bottom, the source-frame total mass distribution $p(M)$, redshift distribution $p(z)$, spin distribution $p(a)$, and mass ratio distribution $p(q)$.}
    \label{fig:detected_catalog_distributions_grid}
\end{figure}

We consider \textit{detected mergers}, binaries that coalesce within the mission's timespan (4 yrs) that produce $\rho>12$. Our counts are shown in Table~\ref{table:detected_merger_rates}. In the case of the light-seed population model popIII, we have $1119$ detected mergers for a 4-year mission, $\approx279$ detections per year. This count is positioned above the results found in the literature, Table~\ref{tab:detected_mergerrates_literature}. The most relatable studies from Table~\ref{tab:detected_mergerrates_literature} with ours are the ones from Klein \textit{et al}.\ (2016), and Barausse \textit{et al}.\ (2023), where they predict lower detections than us, $\sim357$ for a 5-year mission ($\sim71$ detections per year), and $\sim339$ for a 4-year-mission ($\sim84$ detections per year), respectively. This gives us $\sim3.5$ times more detections per year. Regarding the heavy-seeds population models, for Q3d we predict $\sim20$ detections per year, where as Klein \textit{et al}.\ and Barausse \textit{et al}.\ predict $\sim8$ and $\sim18$ per year, respectively. For the Q3nod model, we predict $\sim153$ detections per year, whereas Klein \textit{et al}.\ and Barausse \textit{et al}.\ predict $\sim121$ and $\sim164$ detections per year.

The differences observed in our predictions with the ones in the literature might be explained by several causes. First, our intrinsic merger rates are higher than Klein \textit{et al}.'s by $\sim6\%$. This alone rises our detection rate at least by the same amount. We used Barausse \textit{et al}.\ values to normalise our intrinsic merger rates per redshift, so in principle they should agree. Second, different approaches to generate the catalog of sources. We drew samples from parametric models, Sec.~\ref{subsec:parametricmodels}, with bounded values as justified by the literature. This means that our catalogs are fundamentally different from the sources studied by Klein \textit{et al}.\ which come from semi-analytical models of galaxy/MBH formation. Third, Klein \textit{et al}.\ considers the eLISA-era design with different "\textit{NiAjMkl}" families: different arms lengths, 4 and 6 detector links, mission lifetimes, and different noise prescriptions. On the other hand, our study uses the final LISA baseline, i.e. 2.5 Gm arms with 6 links, and 4-year nominal mission. There are differences in the detector's sensitivity/response too. We use GWFish's updated PSD curve, Eqs.~\ref{redPSDrespa} and \ref{redPSDrespb}, along with updated acceleration, system metrology, and galactic foreground noises. A fourth source of difference are the waveform models. We used the spin-precessing waveform IMRPhenomPv2 which models the inspiral, merger, and ringdown phases. Klein \textit{et al}.\ used inspiral-only precessing waveforms, and rescaled their Fisher matrices to account for the effect of the merger-ringdown. A fifth cause of difference is the counted mergers occuring within the mission.  Several studies, consider signal detected when its SNR is higher than a threshold while they are still in the inspiral phase and not coalescing within the mission time span. In our case, we count detected coalescences as they merge within the mission time span. 
...
\midrule
LS & popIII         & $357.9/5\mathrm{yrs}$ & Klein \textit{et al}.\ (2016). For an optimistic acceleration-noise level, arm length of 2 million km, mission duration of 5 years, and 6 laser links. $\mathrm{SNR}>8$.\\
LS &  popIII        & $339/4\mathrm{yrs} $ & Barausse \textit{et al}.\ (2023). Assuming infinite resolution in models/simulations. \\
LS &  popIII        & $129.3/4\mathrm{yrs} $ & Mangiagli \textit{et al}.\ (2022). Events with $\rho>10$. \\
LS & SN feedback-delays         & $16/4\mathrm{yrs}$ & Barausse \textit{et al}.\ (2020). \\
LS & No SN feedback-delays         & $146/4\mathrm{yrs}$ & Barausse \textit{et al}.\ (2020). \\
LS &          & $\approx75/\mathrm{yr}$ & Bonetti \textit{et al}.\ (2020). \\
HS & Q3d         & $40.8/5\mathrm{yrs}$  & Klein \textit{et al}.\ (2016). For an optimistic acceleration-noise level, arm length of 2 million km, mission duration of 5 years, and 6 laser links. $\mathrm{SNR}>8$.\\
HS &  Q3d        & $74/4\mathrm{yrs} $ & Barausse \textit{et al}.\ (2023). Assuming infinite resolution in models/simulations.\\
HS &  Q3d        & $30.4/4\mathrm{yrs} $ & Mangiagli \textit{et al}.\ (2022). Events with $\rho>10$. \\
HS & Q3nod        & $609.4/5\mathrm{yrs}$ & Klein \textit{et al}.\ (2016). For an optimistic acceleration-noise level, arm length of 2 million km, mission duration of 5 years, and 6 laser links. $\mathrm{SNR}>8$.\\
HS &  Q3nod        & $656/4\mathrm{yrs} $ & Barausse \textit{et al}.\ (2023). Assuming infinite resolution in models/simulations.\\
HS &  Q3nod        & $471.1/4\mathrm{yrs} $ & Mangiagli \textit{et al}.\ (2022). Events with $\rho>10$. \\
HS & No SN feedback-delays         & $10/4\mathrm{yrs}$ & Barausse \textit{et al}.\ (2020). \\
HS & No SN feedback-nodelays         & $1288/4\mathrm{yrs}$ & Barausse \textit{et al}.\ (2020). \\
HS &          & $\approx25~/~\mathrm{yr}$ & Bonetti \textit{et al}.\ (2020). \\
\bottomrule
\end{tabular}
