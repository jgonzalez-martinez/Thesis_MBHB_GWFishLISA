...
Sensitivity curves are used when trying to grasp an idea of what GW sources are at the reach of a particular detector. Figure () shows the sensitivity curve for LISA dependent of the frequency. We can look at how loud will be a signal by looking how high is the signal curve above the sensitivity curve. The sensitivity curve for LISA can be expressed as \cite{robson2019construction},
\begin{equation}
...
\begin{enumerate}
    ...
    \item PhenomB and PhenomC non-precessing spins with single effective spin to account for the two spin degrees of freedom. PhenomP was developed from PhenomC where the "twsting"-up approximation to account for spin precession was included.
	...
	\begin{enumerate} 
        ...
        \item PhenomPv3HM includes higher harmonics with twsiting up precession approach.
        \item Phenom XAS upgrades from PhenomD with imporvements in the dominant harmonic.
        \item Phenom XHM add higher harmonics to PhenomX, and, in turn, PhenomXPHM add spin precession.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item The addition of closed-form anzats which include precession and eccentricity in a large parameter space without compromising computational efficiency.
...
\section{LISA simulated observations of MBHBs with software GWFish}
...
GWFish computes the derivatives of the waveform in a hybrid analytical-numerical fashion. The analytical derivatives of the waveform $\Tilde{h}_{k}(\theta^{j},f)$ with respect to the waveform phase $\varphi_{c}$, the luminosity distance $d_{L}$ and merger time $t_{m}$ computed by GWFish are,
...
To obtain the slopes of $p(\mathcal{M},q)$ we used a different approach: Since we obtained $p(\mathcal{M},q)$ from $p(m1,m2)$ with the jacobian transformation, eq. (\ref{eq:jacobianm1m2chirpMq}), we don't have it's covariance matrix to compute the eigenvectors. We obtain the covariance matrix of $p(\mathcal{M},q)$ from the covariance matrix of $p(m_1,m_2)$ by means of the transformation,
\begin{equation}
