...
There is numerical and theoretical support for light seeds struggling to achieve efficient high accretion \cite{omukai2002upper,oh2003fossil,whalen2004radiation}. Smith \textit{et al.} \cite{smith2018growth} investigated the growth of more than 15,000 light seeds using the Renaissance simulations, and found that they do not grow more than $10\%$ in a time span of $300~\mathrm{Myrs}$. By using a cosmological, data constrained semi-analytical model, Valiante \textit{et al.} \cite{valiante2016first} showed that light seeds struggle to have significant growth.
...
\begin{equation}
\tau_{\mathrm{DF}} \approx \frac{8~\mathrm{Gyr}}{\ln\Lambda} \left(\frac{r}{\mathrm{kpc}} \right)^{2} \frac{\sigma}{200~\mathrm{km/s}} \frac{10^7~\mathrm{M}_{\odot}}{M}.
...
\label{eq:dedt}
\end{equation}
When considering PN coefficients up to order 3.5, or higher, the Keplerian variables of the orbit are not constant anymore, but rather oscillate \cite{will2014confrontation,mannerkoski2019gravitational,memmesheimer2004third}. When including spins, they also participate on shaping the dynamics of the binaries as well as the GWs emitted.
...
Because these signatures are observed through EM spectroscopic surveys, these are generally biased, due to selection effects, to MBHBs with masses of $10^{6}$--$10^{7}~\mathrm{M}_{\odot}$, separations of $\geq0.1$~pc and redshifts of $z\geq1$--$2$ \cite{pflueger2018likelihood,xin2021ultra,nguyen2016emission}. This means that they only cover a subset of MBHBs, which are progenitors to binaries in the LISA band but which will not be detected by LISA due to the long coalescence time-scales involved.
...
The Vera Rubin Observatory will carry out the Legacy Survey of Space and Time (LSST~\cite{ivezic2019lsst}) providing time-domain EM observations of MBHBs through photometric variablity. In particular, MBHBs may manifest as AGNs with periodic or quasi-periodic variablity associated with binary-drive modulation of the accretion flow. LSST is expected to monitor a large number of quasars (up to order a million), with multi-band, high-cadence light curves over long baselines, extending up to 10 yr. This makes it well suited for short-lived, and short period MBHBs emitting GWS in the LISA band.

Spectroscopic surveys such as the Sloan Digital Sky Survey (SDSS-V;~\cite{kollmeier2026sloan}) provide another potential EM route to identify sub-parsec MBHB candidates. If one BH in an MBHB system hosts a broad-line region, its orbital motion can imprint Doppler shifts in the braod emission lines~\cite{nguyen2020emission}, producing time-variable line profiles. However, these signatures are not unique to binaries, so long-term spectroscopic follow-up is required to observe coherent changes in the broad emission lines~\cite{runnoe2017large}.
