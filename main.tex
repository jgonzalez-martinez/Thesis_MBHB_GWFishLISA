...
When estimating the PDF $p(X,Y|I)$ where $X$ and $Y$ are the constrained parameters and $I$ is the background information, we might be interested in the posterior $p(Z|I)$, where $Z$ is derived from $X$ and $Y$, $Z=f(X,Y)$. This process is called $\textit{error-propagation}$, and is performed by means of variable transformation \cite{sivia2006data}. In the case of one variable, we might ask how $p(X|I)$ is related to $p(Y|I)$ if $Y=f(X)$?. Let's say that $\delta X$ is a very small interval about $X=X^{\ast}$, the probability that $X$ lies in the range between $X^{\ast}-\delta X/2$ and $X^{\ast} + \delta X/2$, is
\begin{equation}
...
In order to perform simulations for a set of MBHB observations, we need to ensure that the waveform derivative computations are stable. We performed a series of tests by computing $\sigma^{2}_{m_1}$ and $\sigma^{2}_{m_2}$ directly from the inverse Fisher matrix of $(m_1,m_2)$ for different values of the derivative step size $\epsilon$ using the waveform models \texttt{IMRPhenomXPHM}, \texttt{IMRPhenomD}, and \texttt{IMRPhenomPv2}. The results of these tests are shown in Figs.~\ref{fig:testXPHM},~\ref{fig:testPhenomD}, and~\ref{fig:testPv2}. 

\begin{figure}[htbp]
\includegraphics[width=.8\textwidth]{Figures/variancem1_stepsize_spins02_tilts14_XPHM.pdf}
\caption{Plot to show the stability of the waveform derivatives with \texttt{IMRPhenomXPHM}. The results for $\sigma^2_{m_1}$ are scattered across $\epsilon$ meaning that there is no convergence in the results.}
\label{fig:testXPHM}
\end{figure}

\FloatBarrier
\begin{figure}[htbp]
\includegraphics[width=.8\textwidth]{Figures/variancem1_stepsize_spins02_tilts14_PhenomD.pdf}
\caption{Plot to show the stability of the waveform derivatives with \texttt{IMRPhenomD}. We can see a semi-plateau from $\sim10^{-6}$ to $3\times10^{-5}$ meaning that in that range, the Fisher matrices converge.}
\label{fig:testPhenomD}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}{0.70\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figures/variancem1_stepsize_spins02_tilts14_chirp1e6_q075_PhenomPv2.pdf}
        \caption{$\sigma^{2}_{m_1}$ \textit{vs} $\epsilon$ for PhenomPv2}
        \label{fig:testPv2m1}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.70\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figures/covariancem1m2_stepsize_spins02_tilts14_chirp1e6_q075_PhenomPv2.pdf}
        \caption{$\sigma_{m_1,m_2}$ \textit{vs} $\epsilon$ for PhenomPv2}
        \label{fig:testPv2m2}
    \end{subfigure}    
    \caption{Plot to show the stability of the waveform derivatives with \texttt{IMRPhenomPv2}. We can see a plateau from $\sim10^{-6}$ to $3\times10^{-5}$ meaning that the Fisher matrices converge in that range.}
    \label{fig:testPv2}
\end{figure}
...
In the context of LISA, population inference of MBHBs is possible since LISA is expected to observe MBHB for a range of redshifts, from the local universe up to the early Universe, where galaxies first formed. The observed number of mergers will be then compared with parametric and non-parametric population models which condense information about the MBH formation and evolution channels. The main approach to carry out this analysis is through hierarchical Bayesian framework. 

\noindent\rule{\linewidth}{0.1pt}

\textit{Modelling approaches}. As we have stated previously, there is strong evidence that MBHs and galaxies co-evolve. Approaches to follow this co-evolution include cosmological simulations, theoretical models, and phenomenological models. Cosmological simulations can be carried out in two fashions: Grid-based Eulerian simulations, and particle-based hydrodynamical simulations (Smoothed Particle Hydrodynamics, SPH). While cosmological simulations give a robust track of the evolution of MBHs and their host galaxies, they are computationally expensive. On the theoretical side, there exist analytical and semi-analytical models. Theoretical models are limited in their reach as far as the physics that is included in the modelling. Semi-analytical models provide simple enough prescriptions, tunable up to a few parameters, while being computationally affordable. Phenomenological models consist in models that fit observations. In this work, we are interested in this type of models.

\subsection{Parametric models}
\label{subsec:parametricmodels}

\subsubsection{Merger rate}
\label{subsubsec:mergerrates}

We use the expressions from Wang et al. (2016) \cite{wang2022forecast}:
...
These expressions are plotted in Figure~\ref{fig:mergerrates_Wang2016}.

\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{Parametric_forms/merger_rates_populations.pdf}
\caption{}
\label{fig:mergerrates_Wang2016}
\end{figure}

\subsubsection{Mass distribution}
\label{subsubsec:massdistribution}

We use a broken power law distribution for our primary mass.

\begin{align}
p(m_1 \mid \alpha_1,\alpha_2,m_{\min},m_b,m_{\max})
&\propto
\begin{cases}
 m_1^{-\alpha_1}, & m_{\min} \le m_1 < m_b,\\[6pt]
 m_1^{-\alpha_2}, & m_b \le m_1 \le m_{\max}.
\end{cases}
\end{align}

The broken power law distribution is shown in Fig.~\ref{fig:p_m_brokenPwl}.

\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{Parametric_forms/p_m1_brokenPwl.pdf}
\caption{}
\label{fig:p_m_brokenPwl}
\end{figure}

\subsubsection{Redshift distribution}
\label{subsubsec:redshiftdistribution}

The redshift distribution for the popIII model is shown in Fig.~\ref{fig:p_z_popIII}.

\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{Parametric_forms/p_z_popIII.pdf}
\caption{}
\label{fig:p_z_popIII}
\end{figure}

The redshift distribution for the Q3d model is shown in Fig.~\ref{fig:p_z_Q3d}.

\FloatBarrier
\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{Parametric_forms/p_z_Q3d.pdf}
\caption{}
\label{fig:p_z_Q3d}
\end{figure}

The redshift distribution for the Q3nod model is shown in Fig.~\ref{fig:p_z_Q3nod}.

\FloatBarrier
\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{Parametric_forms/p_z_Q3nod.pdf}
\caption{}
\label{fig:p_z_Q3nod}
\end{figure}
