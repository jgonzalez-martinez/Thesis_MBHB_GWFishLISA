...
The content of the present work is as follows. We will introduce the reader to the theory of massive black holes, their properties, and the astrophysics involved in their formation and evolution in chapter 2. In chapter 3 we will describe the LISA detector; the configuration of the detector, the physical principles under which operates, and the techniques used to detect GWs. In chapter 4 we delve into the elements that are involved in observations and analysis of MBHBs with LISA; the noise properties of the detector, the waveform models used in data analysis and the data analysis techniques. Chapter 5 we introduce the GWFish code which is used to carry out the simulations of the LISA observations we analyze and describe the physical systems under study. Chapter 6 we present our results, the discussion of our results in Chapter 7, conclusions in Chapter 8, and future work in Chapter 9. 

\section{Massive Black Hole Binaries. Theory and Observations}
\label{sec:MBHBTheoObs}

The GW community distinguishes black holes basically in three categories: Stellar black holes, with masses of 6 to 80$~\mathrm{M}_{\odot}$ \cite{amaro2023astrophysics}, which form from the collapse of stars that undergo supernovae explosion. These can be detected through their X-ray, infrared and optical emissions, and certainly by their GW emission when they come in binaries. On the other hand, there exist massive black holes with masses in the range of $\num{e5}$ to $\num{e9}~\mathrm{M}_{\odot}$ which power quasars and active galactic nuclei (AGN), and have been observed in nearby galaxy spheroids and certainly, our Milky Way galaxy host an almost inactive massive black hole. There is a gap between these two categories, called intermediate mass black holes which has not been accurately constrained due to the lack of observations. Stellar black holes, on the other hand, can have masses as large as $\num{e2}$ M$_{\odot}$ depending on the metallicity of the progenitor stars, and on radiation feedback. Through X-Ray observations, there is evidence that massive black holes have grown in mass through episodes of mergers and coalescences driven by galaxy mergers. This has led to the concept of black hole seeds, which masses are theoretically weakly constrained to this day. 

In this chapter we will delve into the formation of massive black hole from seeds, their growth and the influence they have in their host galaxies. We then introduce massive black holes in binaries, their formation after a galaxy merger, evolution and their detectability through the emission of GWs by LISA.
...
\end{equation}
where $\mu$ is the reduced mass of the binary and $\sigma$ the local velocity of the surrounding distribution. At this point, individual stars are ejected from the MBHB neighbor, decreasing its number density. Once the binary hardening regime is reached, the expected orbital shrinking rate is
\begin{equation}
...
The GW-driven evolution of the binary can be described by the seminal work of Peters \cite{peters1964gravitational}. The evolution of the keplerian parameters $a$, the semi-major axis of the orbit, and $e$, the eccentricity, can be tracked down when assuming a slow variation. This is the case when the orbital period scales as $t\sim(a/r_{g})^{3/2}$ and the radiation reaction time-scale scales as $t_{\mathrm{RR}}~(a/r_{g})^{4}$. Noting that $a\gg r_{g}$ implies $t_{\mathrm{orb}}\ll t_{\mathrm{RR}}$, the orbit is Keplerian, and the paramters $a$ and $e$ vary slowly. The orbit average of the evolution of the binary's semi-major axis is described by \cite{peters1964gravitational}
\begin{equation}
...
\label{eq:dedt}
\end{equation}
When considering PN coefficients up to order 3.5, or higher, the dynamics of the orbit is not constant anymore, but rather oscillates (reference). When including spins, they also participate on shaping the dynamics of the binaries (references) as well as the GWs emitted.
...
Trails of GW recoil could be accessible to EM observations through the shining of the gas surrounding the MBHB remnant, which in this case would shine as off-nuclear AGNs.
...
\begin{enumerate}
    ...
    \item Unusual ratios in the broad emission lines due to the tidal forces of one component black hole upon the other component of the candidate binary \cite{montuori2011search,montuori2012search}.
\end{enumerate}
...
For MBHBs with smaller orbital separations compared to the ones involved in optical spectroscopic surveys, Reynolds~\cite{reynolds2015measuring} used the broad iron fluorecence emission lines observed at $\sim6.4 \text{keV}$ in X-rays in the accretion flow of many AGNs with masses of $~10^{6}~\mathrm{M}_{\odot}$. However, this observation is limited to AGN's at lower redshifts compared to LISA's targets, so high sensitivity, high redshift X-ray facilities, like Athena, will be suitable to take full advantage of this approach. 
...
LISA will be able to observe MBHB seeds of $10^{4}-10^{5}~\mathrm{M}_{\odot}$ at high redshifts, opening the possibility of observing the dawn of MBHBs. However, this is out of reach for many of the current EM facilities. EM surveys that have given insights on low-mass MBHs have only been on dwarf galaxies in the local universe ($z\sim2$) with stellar mass $M_{\star} = 10^{7} - 10^{9.5}~\mathrm{M}_{\odot}$\cite{reines2015relations,baldassare201550,mezcua2016population,mezcua2018intermediate}. Dwarf galaxies are of interest in the search for MBH seeds because, unlike massive galaxies which have experienced significant mass growth, dwarf galaxies have experienced less mass growth through cosmic ages \cite{habouzit2017blossoms}, giving more information about how their MBHs formed.  
...
While LISA will explore the millihertz regime of MBHBs, PTAs are able to look at even lower frequencies, at the nanohertz band. PTAs monitor flucutactions in the time variation between pulses emitted from millisecond pulsars over long periods of time. When there is a correlation in the time variations detected among the pulsars in the array, and this correlation follows the quadrupole correlation signature, then a GW has been detected. The main targets of PTAs are MBHBs with masses of $10^8--10^{10}~\mathrm{M}_{\odot}$ at $z\approx1-2$ and the stochastic GW background produced by the superposition of incoherent GW signals. The nanoGRav collaboration with a database of 12.5 years of 47 pulsars showed that a stochastic GW background is consistent with the predictions of a background produced by SMBHs \cite{arzoumanian2020nanograv}. However, there are other possible sources capable of producing this GW spectrum, such as cosmic strings. In this case, longer follow-up and larger pulsar arrays will be needed to confirm this observation. 
...
\end{equation}
In the case of LISA, the difference in the fluctuations will be of order $\sim10^{-13}/\sqrt{\mathrm{Hz}}$. Let's express eq. \eqref{eq:freqfluctdiff} in its frequency domain by means of a Fourier transform,
\begin{equation}
...
\end{equation}
However, in order to suppress the laser noise fluctuations below the LISA sensitivity, the integration time needed would amount to six months. A solution to this was proposed by Tinto \& Armstrong~\cite{tinto1999cancellation} which works in the time domain. By using the new data set, the difference of $y_{1}$ and $y_{2}$ from Eq.~\eqref{eq:dopplerdata1} and Eq.~\eqref{eq:dopplerdata2} is
\begin{equation}
...
\begin{enumerate}
    ...
    \item PhenomXAS upgrades from PhenomD with improvements in the dominant harmonic.
        \begin{enumerate}
        \item Phenom XHM adds higher harmonics to PhenomXAS, and PhenomXPHM adds higher harmonics and spin precession.
        \end{enumerate}
\end{enumerate}
...
\begin{align}
P^{a}_n(f) = P^{e}_n(f) 
&= 16~\mathrm{sin}^{2}\Big(\frac{\pi f L}{c}\Big)\Big[3+2\text{cos}\Big(\frac{2\pi fL}{c}\Big)+ \mathrm{cos}\Big(\frac{4\pi fL}{c}\Big)\Big]S^{\mathrm{pm}}(f) \nonumber \\
&\quad +8\mathrm{sin}^{2}\Big(\frac{\pi fL}{c}\Big)\Big[2+\mathrm{cos}\Big(\frac{2 \pi fL}{c}\Big)\Big]S^{\mathrm{op}}(f),
\label{redPSDrespa} \\
P^{t}_n(f)  
&= \Big[2+4\mathrm{cos}^{2}\Big(\frac{2\pi fL}{c}\Big)\Big]\Big[4\mathrm{sin}^{2}\Big(\frac{\pi fL}{c}\Big)\Big]S^{\mathrm{op}}(f)S^{\mathrm{pm}}(f)\,
\label{redPSDrespb}
\end{align}
\end{subequations}
...
The characteristic noise PSD for the TDI channels $a, e, \mathrm{and} t$, $S^{a,e,t}_c(f)$ is
\begin{equation}
...
\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{figs_Marsat_comparison/Sc_Curves_XPHMSignal_GWFishFormula.pdf}
\caption{Sensitivity curve $S_c$ using GWFish formula, Eq.~\eqref{eq:charPSD}, shown together with Marsat \textit{et al.} characteristic noise PSD in red dots and waveform signal with \texttt{IMRPhenomXPHM} with parameters from Table \ref{t:MBDvalues}.}
...
We used the study by Marsat~\textit{et al.} as a reference to guide and compare our results. A direct comparison of our result for $p(m_1,m_2)$ to the one obtained by Marsat~\textit{et al.} is shown in Fig.~\ref{fig:Marsatcomparison}. The results for $p(m_1,m_2)$, $p(\mathcal{M},q)$, and $p(M,q)$ are shown in Fig.~\ref{fig:PDFsMarsat}. We notice a difference in our result for $p(m_1,m_2)$ against the one obtained in Marsat~\textit{et al.} in Fig~\ref{fig:Marsatcomparison}. This is due to different prescriptions of the noise PSD in GWfish and in Marsat~\textit{et al}. Improvements in this respect can be done by using updated noise PSD curves in GWFish. More on this in Section \ref{sec:discussion}. On the other hand, the results for the PDFs, shown in Fig.~\ref{fig:PDFsMarsat}, are consistent with what is expected from the correlations of the involved parameters: Component masses $(m_1,m_2)$ are negatively correlated where one parameter increases as the other decreases for a fixed measured $M$. The PDF for $(\mathcal{M},q)$ shows positive correlation as expected. The positive correlation between $\mathcal{M}$ and $q$ is shown mathematically in Appendix B. The PDF for $(M,q)$ shows negative correlation. This is also expected for an MBHB with total mass $M=\mathcal{O}(10^6)$: Since the merger happens in the region of the sensitivity curve where the slope is positive, an increasing perturbation in $M$ shifts the signal to lower frequencies. In order to compensate this frequency decrease, the mass ratio $q$ must be decreased so that the higher harmonics associated with lower $q$s, or more unequal mass binaries, power back the signal to higher frequencies.

\begin{figure}[htbp]
\includegraphics[width=.7\textwidth]{figs_Marsat_comparison/pm1m2_MarsatComparison_nospins_GWFish_SNR237.pdf}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering        \includegraphics[width=\textwidth]{figs_Marsat_comparison/singlepm1m2_PhenomXPHM_case1_nospins_GWFish_SNR237.pdf}
        \caption{$p(m_1,m_2)$}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_Marsat_comparison/singlepMq_PhenomXPHM__nospins_GWFish_SNR237.pdf}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_Marsat_comparison/singleptotalMq_PhenomXPHM_nospins_GWFish_SNR237.pdf}
        \caption{$p(M,q)$}
...
We can see that \texttt{IMRPhenomXPHM} yields scattered results for $\sigma^{2}_{m_1}$ for different values of $\epsilon$, showing an unstable performance on the derivative computations. On the other hand, waveforms \texttt{IMRPhenomD} which does not include spin precession nor higher harmonics, and \texttt{IMRPhenomPv2}, which includes spin precession and no higher harmonics, were more stable in their computations with $\epsilon$ values from $~1\times10^{-8}$ to $~3\times10^{-5}$ for \texttt{IMRPhenomD} and $~3\times10^{-6}$ to $~3\times10^{-5}$ for \texttt{IMRPhenomPv2}. We chose to change waveform from \texttt{IMRPhenomXPHM} to \texttt{IMRPhenomPv2}. 

Now that we have the simulation software set up with a waveform, we set a parameter space which LISA is expected to observe with $M$ in the range from $1\times10^{5}~\mathrm{M}_{\odot}$ to $1.7\times10^{6}~\mathrm{M}_{\odot}$ \cite{mangiagli2020observing}. The orientation parameters are set to default GWFish values. PDF contour plots for these are shown in Figs.~\ref{fig:q025Pv2realmass},~\ref{fig:q050Pv2realmass}, and~\ref{fig:q075Pv2realmass} and the slopes plots in Figs.~\ref{fig:slopespm1m2Pv2realmass},~\ref{fig:slopespchirpMqPv2realmass}, and~\ref{fig:slopesptotalMqPv2realmass}.

\section{Results}
\label{sec:results}

Results for the PDFs of a single MBHB observation are shown in Figs~\ref{fig:q025Pv2realmass},~\ref{fig:q050Pv2realmass}, and~\ref{fig:q075Pv2realmass} for $q=0.25,~0.50,~0.75$ respectively, for a binary system with $M=6.5\times10^5~\mathrm{M}_{\odot}$. The PDFs from this event shares much of the features seen in Fig. ~\ref{fig:PDFsMarsat} from the comparison with Marsat \textit{et al}. However, we notice a curved PDF ellipse for $p(\mathcal{M},q)$ that we will explain later in this section.

The results for the PDF orientation analyses of the set of 50 events for a parameter space in the $M$ range between $1\times10^5~\mathrm{M}_{\odot}$ and $1.7\times10^6~\mathrm{M}_{\odot}$ are shown in Figs.~\ref{fig:slopespm1m2Pv2realmass},~\ref{fig:slopespchirpMqPv2realmass}, and~\ref{fig:slopesptotalMqPv2realmass}. It is worth mentioning that the parameter space considered initially a higher limit in total mass of $5\times10^{6}~\mathrm{M}_{\odot}$ but was reduced to $1.7\times10^{6}~\mathrm{M}_{\odot}$ because of issues in computing the Fisher matrices for total masses above $1.7\times10^{6}~\mathrm{M}_{\odot}$, where uncertainties were the same order of magnitude as the true values or even exceeding these by one order of magnitude.
For  $p(m_1,m_2)$ in Fig.~\ref{fig:slopespm1m2Pv2realmass} we notice that the slopes for more unequal mass systems cover a wider range than for more equal masses, meaning that the uncertainty estimations are less stable for more unequal mass systems. This is expected since the waveform signal is dominated by $m_{1}$; the measurements of $m_{2}$ are more uncertain. This leads to more variation in the slopes across chirp mass. At lower masses, chirp mass, which is a combination of $m_{1}$ and $m_{2}$, dominates the evolution. In this regime, the slopes (and the correlations) are weak because $m_{1}$ dominates the evolution over $m_{2}$. This means that small changes in $m_{2}$ do not impact the waveform significantly. As the total mass increases, $m_{2}$ becomes more important, leading to stronger correlations. As mass ratio increases as in the cases for $q=0.50$ and $q=0.75$, there is less variation in the slopes across chirp mass, as both $m_{1}$ and $m_{2}$ have important contributions to the waveform signal. In this case, $m_{1}$ and $m_{2}$ are more correlated.

Regarding the slopes of $p(\mathcal{M},q)$, after showing negative slopes at lower chirp mass, all mass ratios show positive slopes as the chirp mass increases, as expected. Higher chirp mass yield more similar results in the orientations of the PDF contour plots for all mass ratios and uncertainty measurements are more reliable for higher chirp mass. 

The slopes plot of $p(M,q)$ shows mainly negative slopes. However, as the chirp mass increases, there is a shift from negative to positve slopes. This has to do with the region on the sensitivity curve where the merger occurs. On the other hand, largest changes in the uncertainty estimations occur for more equal systems, $q=0.75$ and smaller changes for more unequal systems. 

Going back to the $p(\mathcal{M},q)$ PDF in Fig.~\ref{fig:pMcq075} where one can notice a curvature in the PDF ellipse. This means that the uncertainties' correlations between $\mathcal{M}$ and $q$ are non-linear. This effect can be seen more accentuated in Figs.~\ref{fig:curvedPDFs_q025}, ~\ref{fig:curvedPDFs_q050}, ~\ref{fig:curvedPDFs_q075}. for different values of $q$, each of them showing PDFs for different values of the total mass $M$. From these figures we can notice how the curvature increases with mass ratio and total mass. For lower masses, the Jacobian transformation from $(m_1,m_2)$ to $(\mathcal{M},q)$ can be considered valid for a range of mass ratios, but as the total mass and mass ratio increase, the transformation need to be reparametrized so that it maintains linearity \cite{cutler1994gravitational}.

\begin{figure}
    \centering
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_PhenomPv2_realmass/pm1m2_17_m1_524100.0_m2_131000.0.pdf}
        \caption{$p(m_1,m_2)$}
        \label{fig:pm1m2025}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_PhenomPv2_realmass/pchirpMq_17_Mc_218147.25540413498_q_0.2499522991795459.pdf}
        \caption{$p(\mathcal{M},q)$}
        \label{fig:pMcq025}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_PhenomPv2_realmass/ptotalMq_17.pdf}
        \caption{$p(M,q)$}
        \label{fig:pMq025}
    \end{subfigure}    
    \caption{PDFs for a single MBHB observation with $q=0.25$ and $M=6.5\times10^5~\mathrm{M}_{\odot}$.}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_PhenomPv2_realmass/pm1m2_17.pdf}
        \caption{$p(m_1,m_2)$}
        \label{fig:pm1m2050}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_PhenomPv2_realmass/pchirpMq_17.pdf}
        \caption{$p(\mathcal{M},q)$}
        \label{fig:pMcq050}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_PhenomPv2_realmass/ptotalMq_17.pdf}
        \caption{$p(M,q)$}
        \label{fig:pMq050}
    \end{subfigure}    
    \caption{PDFs for a single MBHB observation with $q=0.50$ and $M=6.5\times10^5$.}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q075_PhenomPv2_realmass/pm1m2_17.pdf}
        \caption{$p(m_1,m_2)$}
        \label{fig:pm1m2075}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q075_PhenomPv2_realmass/pchirpMq_17.pdf}
        \caption{$p(\mathcal{M},q)$}
        \label{fig:pMcq075}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q075_PhenomPv2_realmass/ptotalMq_17.pdf}
        \caption{$p(M,q)$}
        \label{fig:pMq075}
    \end{subfigure}    
    \caption{PDFs for a single MBHB observation with $q=0.75$ and $M=6.5\times10^5$.}
...
\begin{figure}[htbp]
\includegraphics[width=.8\textwidth]{figs_slopes_realMasses_PhenomPv2/chirpMq_slope_chirpMq_zoomin.pdf}
\caption{Plot of slopes of $p(\mathcal{M},q)$ with \texttt{IMRPhenomPv2} for a set of 50 observations of MBHBs with masses between $1\times10^{5}~\mathrm{M}_{\odot}$ and $1.7\times10^{6}~\mathrm{M}_{\odot}$.}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_curvedPDFs/pchirpMq_10_Mc_142042.90150697014_q_0.2500293083235639.pdf}        \caption{$M=4.26\times10^5~\mathrm{M}_{\odot}$}
        \label{fig:curvedq025_10}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_curvedPDFs/pchirpMq_30_Mc_359514.7904557554_q_0.24997105476438577.pdf}        \caption{$M=1.07\times10^6~\mathrm{M}_{\odot}$}
        \label{fig:curvedq025_30}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q025_curvedPDFs/pchirpMq_49_Mc_566136.1810332733_q_0.25.pdf}       \caption{$M=1.70\times10^5~\mathrm{M}_{\odot}$}
        \label{fig:curvedq025_49}
    \end{subfigure}    
    \caption{Contour plots of $p(\mathcal{M},q)$ for a single MBHB observation with $q=0.25$ for different values of $M$.}
    \label{fig:curvedPDFs_q025}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_curvedPDFs/pchirpMq_10.pdf}        \caption{$M=4.26\times10^5~\mathrm{M}_{\odot}$}
        \label{fig:curvedq050_10}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_curvedPDFs/pchirpMq_30.pdf}        \caption{$M=1.07\times10^6~\mathrm{M}_{\odot}$}
        \label{fig:curvedq050_30}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q050_curvedPDFs/pchirpMq_49.pdf}        \caption{$M=1.70\times10^5~\mathrm{M}_{\odot}$}
        \label{fig:curvedq050_49}
    \end{subfigure}    
    \caption{Contour plots of $p(\mathcal{M},q)$ for a single MBHB observation with $q=0.50$ for different values of $M$.}
    \label{fig:curvedPDFs_q050}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q075_curvedPDFs/pchirpMq_10.pdf}        \caption{$M=4.26\times10^5~\mathrm{M}_{\odot}$}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figs_q075_curvedPDFs/pchirpMq_49.pdf}        \caption{$M=1.70\times10^5~\mathrm{M}_{\odot}$}
        \label{fig:curvedq075_49}
    \end{subfigure}    
    \caption{Contour plots of $p(\mathcal{M},q)$ for a single MBHB observation with $q=0.75$ for different values of $M$.}
    \label{fig:curvedPDFs_q075}
\end{figure}

\section{Discussion}
\label{sec:discussion}

As we noticed from Fig.~\ref{fig:Marsatcomparison}, our result for $p(m_1,m_2)$ does not match exactly Marsat \textit{et al.} results. This is because of the prescriptions of the PSD noise formulas. Indeed, Marsat \textit{et al.} includes a formula for the PSD, see Eq. $33$ in their paper, and after comparing performances of GWFish and Marsat formulas, we chose GWFish's. It is important to emphasize that the GWFish version used in this work is 1.0.0. A more recent version includes updated PSD noise curves with updated noises for the optical metrology system, $S_{\mathrm{opt}}$, and for the test masses, $S_{\mathrm{pm}}$ so it would be worth considering using updated formulas for future work.
Regarding the simulations of the set of 50 observations, we decreased the original $M$ upper limit from $6\times10^6~\mathrm{M}_{\odot}$ to $1.7\times10^6~\mathrm{M}_{\odot}$. $M$ with values above $1.7\times10^6~\mathrm{M}_{\odot}$ yield uncertainties of the order of magnitude as the true injected value for $m_1$ and $m_2$ so we discarded that regime from our simulations.

\section{Conclusions}
\label{sec:conclusions}

In this work we explored the capabilities of LISA of observing MBHBs using simulated observations with software GWFish. The first step in this process was to simulate a single observation with intrinsic and extrinsic parameter values given in Marsat~\textit{et al} \cite{marsat2021exploring}. The results of these simulations were the Fisher matrices, which contain the variances and covariances of the parameters involved in the simulation, from which the uncertainties are extracted. For the comparison with Marsat~\textit{et al.}, we computed the PDFs $p(m_1,m_2)$, $p(\mathcal{M},q)$ and $p(M,q)$. The results for $p(m_1,m_2)$ show a disagreement which are explained by different prescriptions of the PSD noise of the detector in GWFish and in Marsat~\textit{et al.}, however, we suspect additional inconsistencies in the integration process for computing the SNR. This can be seen in figs. x and x where two waveform signals are plotted, together with the characteristic noise PSD used, and the SNR obtained in each case in the legend box. To our consideration, the SNR obtained in both cases is too low. The results for $p(\mathcal{M},q)$ and $p(M,q)$, on the other hand, agree with what is expected about the correlations between these parameters. Regarding the study of the set of 50 MBHB observations, the results for the PDF agree with what is expected about the correlations between these parameters, however, the parameter space covered in this, $1\times10^5~\mathrm{M}_{\odot}$ to $1.7\times10^6~\mathrm{M}_{\odot}$, set was selected to be much narrower than the observation window of LISA which is $10^4~\mathrm{M}_{\odot}$ to $10^7~\mathrm{M}_{\odot}$. This is due to the Fisher matrices yield uncertainties, with same orders of magnitude as the true values, and even one order of magnitude higher. 

\section{Future work}
\label{sec:futurework}

The original objective of the present work was to set out the statistical tools to carry out population studies of MBHBs with LISA. To do this, these tools need to be robust. A sensible follow-up is to look into the newer versions of GWFish and corroborate the addition of sensible PSD curves and robust prescriptions for the SNR integrations to ensure sensible results, which were the areas where we identified room for improvement. With these elements in place, then GWFish can be the starting point for simulating MBHB observations where the Fisher matrices as outputs are used for computing hyperparameters and merger rates, quantities associated with population inference.
