...
One promising EM signature arises during the late-inspiral of an MBHB, when a circumbinary disc, and two mini discs surrounding each BH can form. These mini-discs are expected to emit large amounts of X-ray radiation~\cite{haiman2017electromagnetic}. Orbital motion of the binaries can produce modulations in the X-ray emission and in the accretion rate, which might be phase-coherent (or nearly so) with the LISA GW signal. Such modulations can help refine the sky localisation, and hence the host galaxy identification, provided by LISA observations~\cite{tang2018late,dal2019detectability}.
...
\paragraph{Current knowledge about MBHBs with current and future missions.}
...
Single GW observations of MBHBs provide insight into the physics of individual systems, i.e.\ the frequency and phase evolution of the binaries, uncertainties, correlations between parameters and degeneracies, detector response to GW signals, and so on. However, a deeper understanding of their formation and evolution is achieved by analysing ensembles, or populations, of MBHB observations. An MBHB population is the ensemble of MBHBs produced by a given formation and evolution scenario, and is characterised by the intrinsic merger rate and binary parameter distributions. In this study, we forecast LISA's ability to observe MBHBs by simulating a LISA mission and inferring both individual-event parameters and population properties under a set of assumed astrophysical models.

In a real LISA mission, we would perform the inverse process: We would observe a collection of MBHB mergers and statistically infer both the individual-event parameters and population hyper-parameters. These inferred population properties, together with the merger rate, can be used to discriminate between different scenarios for how MBHs form and evolve. 

In the next sections, we will go through the key elements that comprise the estimation of merger rates of MBHBs. We start in Section~\ref{subsec:mergerrates} with the mathematical definition of merger rate and its components, and the prescriptions we will use to estimate the merger rates for different MBHB population models. Then, in section ~\ref{subsec:parametricmodels} we will define our phenomenological, or parametric, models for key binary parameters such as component masses, spins, redshift, mass ratio, that the binaries in our simulated population will exhibit. In Section ~\ref{subsec:synth_catalogs_intrinsic_mbhbs}, we will estimate the detected merger rates by simulating detected MBHBs with GWFish and will estimate the uncertainties of the binary parameters in Section~\ref{subsec:synth_catalogs_det_mbhbs}. 

\FloatBarrier
\subsection{Astrophysical models}
\label{subsec:astro_models}

In order to estimate the number of mergers detectable by LISA, we must adopt an MBH formation scenario, i.e.\ an~\textit{astrophysical model}, that provides a theoretical description of how MBHs form and evolve. Different models predict distinct merger rates and different distributions of MBHB parameters (e.g.\ masses, mass ratios, redshifts, and spins). The predicted merger rates are particularly sensitive to 1) the properties of high-redshift seeds (light versus heavy seeds, and the MBH occupation fraction in DM haloes) and 2) the efficiency and timescales of MBH pairing and coalescence following galaxy mergers, which depend on galaxy-MBH delays, the stellar and gaseous environment, and, in some scenarios, merger triggering or acceleration via triple interactions.

As discussed in Section~\ref{}, MBHs may originate from multiple scenarios, including primordial BH seeds, light and heavy seeds, stellar runaway collisions, and hierarchical BH mergers. Light- and heavy-seed scenarios have been studied extensively using cosmological simulations and semi-analytical models, and more recently in the context of GW observations, through several iterations on the configuration and performance of LISA~\cite{sesana2011reconstructing,klein2016science}.

Within models that incorporate light and/or heavy seeds, three classes of physical mechanisms are commonly invoked: 1) delays between DM halo mergers and galaxy mergers, and between galaxy mergers and the MBH-MBH merger~\cite{antonini2015coevolution,antonini2015imprint}; 2) AGN and supernova feedback, which regulate gas supply, and MBH growth~\cite{barausse2020massive,barausse2012evolution,barausse2014erratum}; and 3) three- and four-body interactions, which can promote mergers by efficiently extracting energy and angular momentum from the inner binary~\cite{bonetti2016post,bonetti2018post}.

In this work, we adopt three benchmark population models: a light-seed PopIII scenario, and two heavy-seed Q3 scenarios. The Q3 models are considered in two variants to isolate the impact of galaxy-MBH merger delays:~\textit{Q3d}, which includes a delay between the merger of the host galaxies and the coalescence of their MBHs, and \textit{Q3nod}, which neglects this delay. Although these models rely on simplified astrophysical prescriptions, they are widely used benchmarks in the literature, and provide a controlled set of scenarios for assessing LISA's observational capabilities. 

\FloatBarrier
\subsection{Parametric Forms}
\label{subsec:parametricforms}

As discussed above, there is strong evidence that MBHs and galaxies co-evolve. Approaches to study this co-evolution include cosmological simulations, theoretical, and phenomenological models. Cosmological simulations can be carried out in two fashions: grid-based Eulerian simulations~\cite{teyssier2002cosmological}, and particle-based hydrodynamical simulations (Smoothed Particle Hydrodynamics, SPH)~\cite{lucy1977numerical,gingold1977smoothed}. While cosmological simulations give a robust track of the evolution of MBHs and their host galaxies, they are computationally expensive. On the theoretical side, there exist analytical and semi-analytical models. Theoretical models are limited in their reach as far as the physics that is included in the modelling. Semi-analytical models provide simple enough prescriptions, tunable up to a few parameters, while being computationally affordable. Phenomenological models, often represented by parametric forms, consist of models that fit observations or the predictions of numerical simulations. In this work, we use phenomenological parametric models because they are flexible and allow us to fit the results of semi-analytical models without requiring us to model explicitly the physical mechanisms that generate the corresponding distributions, and because they can be sampled quickly and at low computational cost.
...
\noindent\textit{Intrinsic Merger Rates}

\paragraph{}
We define the intrinsic merger rate~$\lambda$ as the number of mergers $N$ that happen in the Universe per unit time, 
\begin{equation}
  \lambda
  \;=\;
  \int  R(z)dz,
...
\end{equation}
where $dV_c/dz$ is the comoving volume element, $\mathcal{R}(z)$ is the redshift-dependent merger rate density, and $1/(1 + z)$ converts to observer frame. The redshift-dependent merger rate density $\mathcal{R}(z)$ is an intrinsic, local quantity that reflects the number of mergers per comoving volume, per comoving time, and depends on the astrophysical model. On the other hand, the redshift-dependent merger rate $R(z)$ is the global number of mergers that happen in the Universe per unit redshift per unit detector-frame time.
...
After a potential detection of a stochastic GW background by PTA observatories EPTA, InPTA, NANOGrav, PPTA, and CPTA in 2023, constrains in the merger rates of MBHBs have been possible. We can use this new information to update the expressions from W22 and K16, as they are now out of date. We do this by normalising them to the results for the merger rates from Barausse \textit{et al}.\ (2023, B23)~\cite{barausse2023implications}, which takes these constraints into account. 
...
where $\mathcal{N}=\int_{z_{min}}^{z_{max}}R(z)dz$ is the normalisation factor. The redshift distributions for the popIII, Q3d, and Q3nod population models are shown in Fig.~\ref{fig:row_pz}.
\\
...
$t$ & Geocentric coalescence time & $U(0,t_{max})$ \\
\bottomrule
\end{tabular}
...
To construct a catalogue of detected MBHB mergers, we must account for the impact of noise on the idealised signals. Since we are working in the high-SNR regime, we can approximate the measurement uncertainties with a multivariate Gaussian with covariance given by the inverse Fisher matrix. For each source in our Fisher matrix catalogue (Sec.~\ref {subsec:synth_catalogs_intrinsic_mbhbs}), we draw a random sample from this distribution to obtain a noise-perturbed parameter vector $\theta'$, and use $\theta'$ for a new GWFish run. We then recompute the SNR and Fisher matrix for these noise-perturbed injections, producing a catalogue of Fisher matrices that incorporates the effect of instrumental noise into the signal. We use this catalogue to construct a catalogue of $\textit{detected mergers}$.

We consider detected mergers, binaries that coalesce within the mission's timespan (4 yrs) that produce an $\mathrm{SNR}>12$. Our counts and the population distributions are shown in Table~\ref{table:detected_merger_rates} and Fig.~\ref{fig:detected_catalog_distributions_grid}, respectively. In the case of the light-seed population model popIII, we have $179$ detected mergers for a 4-year mission, around $45$ detections per year. This count is positioned below the results found in the literature, Table~\ref{tab:detected_mergerrates_literature}. The most relatable studies from Table~\ref{tab:detected_mergerrates_literature} with ours are the ones from Klein \textit{et al}.\, and Barausse \textit{et al}.\, where they predict higher detections than us, $357$ for a 5-year mission (around $71$ detections per year), and $339$ for a 4-year-mission (around $84$ detections per year), respectively. We obtain fewer detections because our SNR threshold is higher than those adopted in K16 and B23. Regarding the heavy-seeds population models, for Q3d we predict $52$ detections for a 4-year mission, that is around $13$ detections per year,  whereas Klein \textit{et al}.\ and Barausse \textit{et al}.\ predict $8$ and $18$ per year, respectively. For the Q3nod model, we predict $646$ for a 4-year mission (around $161$ detections per year), whereas Klein \textit{et al}.\ and Barausse \textit{et al}.\ predict $121$ and $164$ detections per year, respectively.
...
The differences between our predictions and those in the literature might be explained by several factors. First, in our implementation, the intrinsic merger rate is modelled as a function of redshift only, $\mathrm{d}\dot N/\mathrm{d}z$. This means that correlations between total mass and redshift are not captured explicitly. By normalising $\mathrm{d}\dot N/\mathrm{d}z$ to the rates of B23 we reproduce the correct marginal redshift distribution (and therefore the correct event counts), but a fully self-consistent prediction would require a mass-dependent rate $\mathrm{d}\dot N/\mathrm{d}z\mathrm{d}M$ (or equivalently a conditional mass distribution $p(M|z)$). Second, different approaches to generating the catalogue of sources. We drew samples from parametric models, Sec.~\ref{subsec:parametricforms}, with bounded values as justified by the literature. This means that our catalogues are fundamentally different from the sources studied by Klein \textit{et al}.\ which come from semi-analytical models of galaxy/MBH formation. Third, Klein \textit{et al}.\ considers the eLISA-era design with different "\textit{NiAjMkl}" families: different arm lengths, 4 and 6 detector links, mission lifetimes, and different noise prescriptions. On the other hand, our study uses the final LISA baseline, i.e. 2.5 Gm arms with 6 links, and a 4-year nominal mission. There are differences in the detector's sensitivity/response too. We use GWFish's updated PSD curve, Eqs.~\ref{redPSDrespa} and \ref{redPSDrespb}, along with updated acceleration, system metrology, and galactic foreground noises. A fourth source of difference is the waveform models. We used the spin-precessing waveform IMRPhenomPv2, which models the inspiral, merger, and ringdown phases. Klein \textit{et al}.\ used inspiral-only precessing waveforms, and rescaled their Fisher matrices to account for the effect of the merger-ringdown. A fifth cause of difference is the counted mergers occurring within the mission.  Several studies consider a signal detected when its SNR is higher than a threshold while they are still in the inspiral phase and not coalescing within the mission time span. In our case, we count detected coalescences as they merge within the mission time span. 
...
%\FloatBarrier
The capabilities of LISA to observe MBHBs can be assessed quantitatively using 1) the population parameter distributions of the intrinsic and detected mergers together with a SNR contour ("waterfall") plot, and 2) plots of fractional and absolute parameter uncertainties for each astrophysical model. 

Fig.\ref{fig:waterfall_plot} shows LISA's detection horizon in the $(M_\mathrm{tot},z)$ plane, with the dotted curve indicating our detection threshold $(\mathrm{SNR} = 12)$. Combining this contour with our parametric ranges ($0\le z \le 19~\mathrm{and}~10^{3} \le M_{\mathrm{tot}}/M_{\odot} \le 10^{8}$), we see that detectability is highest for systems in an intermediate mass range, while binaries that are much lighter or much heavier are more likely to fall below threshold. This is consistent with our results of the intrinsic and detected population distributions which follow very similar trends, indicate that most of our intrinsic mergers already lie within LISA's detectable region and that selection effects are therefore weak for the portion of the parameter space populated by our models.

Regarding measurement precision, Fig.~\ref{fig:uncertainties_plot} shows the fractional uncertainties of the component masses $m_1$ and $m_2$, and luminosity distance $d_{L}$, as well as the absolute uncertainties on the spin magnitudes $a_1$ and $a_2$ for the set of detected mergers for the three population models. In each violin plot, the dashed horizontal lines indicate the median (central line) and 0.5th and 99.5th percentiles of the distribution.
Regarding the fractional uncertainties of $m_1$ and $m_2$, we notice that the median in the three scenarios lies between $10^{-2}$ and $10^{-3}$ fractional uncertainty. This is expected, and aligns with the LISA Science Investigations (SI 2.1) on constraining component masses of MBHBs at percentage-level\cite{colpi2024lisa}. The popIII and Q3nod scenarios are shifted slightly towards higher uncertainties of order $10^{-2}$ compared to Q3d, which is consistent as popIII produces weaker signals than Q3d/Q3nod. For events near the threshold SNR, the uncertainty measurements can be rather optimistic, so our results can be considered as a Cramer-Rao lower-bound uncertainties\cite{cutler1994gravitational}. This level of precision is required to distinguish whether a merger lies in the regime associated with light or heavy seeding scenarios\cite{klein2016science}. 
The absolute uncertainties on the spin magnitudes $a_1$ and $a_2$ concentrate on the tens-of-percent precision for the three scenarios, where $a_1$ is better constrained than $a_2$. Results in \cite{colpi2024lisa} for systems with component masses of $10^5\mathrm{M}_{\odot}$ to $10^7\mathrm{M}_{\odot}$ at $z=2$ with high SNRs ($\sim10^3$) report spin uncertainties at the percentage level. Our systems yield lower SNR and higher redshift, so larger uncertainties are expected.
Luminosity distance fractional uncertainties are tens-of-percentage level, higher than those reported in \cite{colpi2024lisa}. This is expected due to amplitude degeneracies with inclination and polarisation, and correlations with sky location; including higher harmonics can help break these degeneracies and improve both sky location and distance\cite{klein2016science}.
